...
template<typename T>
concept HasAsyncWrite = requires(T a) {
    {
        a.serde_async_write(std::declval<std::add_lvalue_reference_t<iobuf>>())
        } -> std::convertible_to<ss::future<>>;
};
...
template<HasAsyncWrite T>
void write_envelope(iobuf& out, T const& t) {
    t.serde_write(out);
}
